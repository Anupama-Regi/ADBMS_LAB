...
create table jobs(job_id int not null auto_increment,job_title varchar(50) not null,min_salary decimal(8,2) not null,max_salary decimal(8,2) not null,primary key(job_id));
create table regions(region_id int auto_increment primary key not null,region_name varchar(50)not null);
create table countries(country_id varchar(2) primary key not null,country_name varchar(40)not null,region_id int,foreign key (region_id) references regions(region_id));
create table locations(location_id int not null primary key,street_address varchar(40) not null,postal_code varchar(20),city varchar(20) not null,state_province varchar(20),country_id varchar(2) not null,foreign key(country_id)references countries(country_id));
create table departments(department_id int auto_increment not null primary key,department_name varchar(20) not null,location_id int not null,foreign key(location_id)references locations(location_id));
create table employees(employee_id int not null auto_increment,first_name varchar(20)not null,last_name varchar(20) not null,email varchar(50)not null,phone_number varchar(12),hire_date date not null,job_id int not null ,salary int not null,manager_id int,department_id int not null,primary key(employee_id),foreign key(job_id)references jobs(job_id),foreign key(department_id)references departments(department_id));
create table dependents(dependent_id int not null auto_increment primary key,first_name varchar(20)not null,last_name varchar(20)not null,relationship varchar(20)not null,employee_id int,foreign key (employee_id) references employees(employee_id));
